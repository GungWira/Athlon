...
export default function CustomerArenaDetail() {
  ...
  const toggleTime = (fieldId, time) => {
    setSelectedTimes((prev) => {
      ...
      return {
        ...prev,
        [fieldId]: alreadySelected ? current.filter((t) => t !== time) : [...current, time],
      }
    })
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      for (const detail of selectedDetails) {
        ...
        if ("err" in result) {
          alert(`Booking gagal pada lapangan ${detail.fieldName}: ${result.err}`)
          return
        }
      }
  ...
  const getAvailableTimesCount = (field) => {
    ...
    const availableTimes = field.availableTimes.filter((time) => !bookedTimes.includes(time))
          ...
          <>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="md:col-span-2 space-y-6">
                <div>
                  ...
                  <div className="flex gap-2 mt-3">
                    {arenaData.sports.map((sport, key) => (
                      ...
                      >
                        <span className="bg-gray-200 p-1 rounded-full">{getSportInfo(sport)}</span>
                        {sport}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">{arenaData.description || "deskripsi tidak ada"}</p>
                </div>
              ...
              <div>
                <div className="text-white p-4 rounded-lg bg-indigo-600">
                  ...
                  <Button variant="secondary" className="w-full" rounded="md" icon={<MapPinned />}>
                    Lihat lokasi
                  </Button>
                </div>
              </div>
            ...
            <div className="mt-8 ">
              ...
              {fieldData.map((field, index) => {
                ...
                return (
                  <div key={index} className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div className="gap-4 mb-4">
                      <div className="md:col-span-2">
                        ...
                        <div className="mb-3">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="flex items-center gap-1">
                              ...
                              <span className="text-gray-600/60"><EqualApproximately className="h-4" /></span>
                              <span className="text-gray-600/60">
                                ICP {field.price}
                                {icpIdrRate ? "IDR " + Math.floor(icpIdrRate * (Number(field.price) / 10000000)) : "Tidak tersedia"}
                              </span>
                            </span>
                          </div>
                        </div>
                        ...
                        >
                          ...
                          <span className="ml-1">
                            {isTimeSlotsVisible ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                          </span>
                        </button>
                    ...
                    {isTimeSlotsVisible && (
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 transition-all duration-300">
                        {field.availableTimes.map((time, idx) => {
                          const isSelected = selectedTimes[field.id]?.includes(time) || false
                          const isBooked = bookedTimes.includes(time)

                          return (
                            <div key={idx} className="border-indigo-600/20 border rounded-md overflow-hidden">
                              <button
                                onClick={() => !isBooked && toggleTime(field.id, time)}
                                ...
                                disabled={isBooked}
                              >
                                <div className="py-2 text-center text-base font-semibold">60 Menit</div>
                                {time}
                              </button>
                            </div>
                          )
