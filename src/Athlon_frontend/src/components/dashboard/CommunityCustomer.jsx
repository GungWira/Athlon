...
export default function CommunityCustomer({ datas, userData }) {
  ...
  useEffect(() => {
    if (datas) {
      setfilteredCommunity(datas.communities);
    }
  }, [datas]);

  useEffect(() => {
    const filtered = datas.communities.filter((community) =>
      community.name.toLowerCase().includes(searchQuery.toLowerCase())
    );
    setfilteredCommunity(filtered);
  }, [searchQuery, datas.communities]);
        ...
        {filteredCommunity.length == 0 ? (
          <div className="w-full px-4 py-5 aspect-video flex justify-center items-center gap-2 flex-col">
            ...
            <p className="text-sm text-[#202020]/70 text-center">
              Yah belum ada nama komunitas yang sesuai dengan kata kunci yang
              kamu input nih!
            </p>
            <Link
              to={"/community/create"}
          ...
          <div className="grid grid-cols-3 justify-start items-start w-full">
            {filteredCommunity.map((community, key) => (
              <CardComunity
                key={key}
                id={community.id}
